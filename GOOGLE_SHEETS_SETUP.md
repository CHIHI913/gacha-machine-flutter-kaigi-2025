...
2. シートタブの名前を **「景品マスタ」** に変更します（Apps Scriptの `SHEET_NAME` と一致させるため）。
3. 1行目に以下のヘッダーを設定します。

| A列 | B列 | C列 | D列 | E列 | F列 | G列 | H列 |
|-----|-----|-----|-----|-----|-----|-----|-----|
| ID | 名前 | 画像URL | 在庫（残数） | 仕入れ総数 | 説明 | 順番 | 作成日時 |

4. サンプルデータを入れる場合の例：

| ID | 名前 | 画像URL | 在庫 | 仕入れ総数 | 説明 | 順番 | 作成日時 |
|----|------|---------|------|------------|------|------|----------|
| sample-1 | サンプル景品 | https://placehold.co/200 | 10 | 20 | テストデータ | 1 | 2025-01-01 |

> **順番列について**：小さい数字のものほどアプリ内で左（上）に表示されます。未設定の場合は作成日時順で補完されます。

5. 同じスプレッドシート内に **「ガチャ結果」** というシートを追加し、1行目に以下のヘッダーを設定します（Apps Script内の `RESULT_SHEET_NAME` で名称変更可）。

| A列 | B列 | C列 |
|-----|-----|-----|
| 景品 | 引いた時間 | 該当景品の残りの個数 |

このシートに、ガチャを回した記録が自動追記されます。
...
`google-apps-script/Code.gs` の内容をコピーしてApps Scriptエディタに貼り付けます。必要に応じて `SHEET_NAME`（景品マスタのシート名）と `RESULT_SHEET_NAME`（ガチャ結果ログのシート名）を実際のタブ名に変更してください。
...
5. 同じくガチャ実行のたびに「ガチャ結果」シートへ「景品 / 引いた時間 / 残り個数」が追記されるか確認。
